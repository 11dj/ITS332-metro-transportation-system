        ...
        <section class="content">
		  ...
		  <div class="box box-warning">
                <div class="box-header with-border">
                  <h3 class="box-title">Add Route and Schedule (step 2)</h3>
                </div><!-- /.box-header -->
                <div class="box-body">
				...
				?>
                  ...
                  <form action="staffaddrouteandscheduleprocess.php" method="get">
					...
					<input type="hidden" name="totalstep" value=<?=$totalstep?>>
					<input type="hidden" name="totalstartingtime" value=<?=$totalstartingtime?>>
					<input type="hidden" name="routename" value=<?=$routename?>>
					<label>Please don't have same station on same route or the pathfinder algorithm will break.</label><br>
					<?php
						for($i=1;$i<=$totalstep;$i++){ ?>
							<label>Select station for step </label><?=$i?>
							<select class="form-control" name="stationstep<?=$i?>">
							<?php
								$qstation = "select * from station where StationTypeID=$stationtype";
								$resultstation = $mysqli->query($qstation);
								while($rowstation = $resultstation->fetch_array()){
							?>
							<option value="<?=$rowstation['StationID']?>"><?=$rowstation['StationNameEN']?></option>
						<?php } ?>
					</select>
					<label>TimeToNextDestinationInSeconds : </label><input type="text" class="form-control"  name="timetonextdest<?=$i?>">
					<label>TimePauseAtStation : </label><input type="text" class="form-control"  name="timepauseatstation<?=$i?>">
					<label>Cost : </label><input type="text" class="form-control"  name="cost<?=$i?>">
					<br>
					<?php    } ?>
					<br>
					<label>Number of starting time<br>
					Please enter in format HH:MM:SS like 15:35:59.<br> </label>
					<?php
						for($i=1;$i<=$totalstartingtime;$i++){?>
							<label>Starting Time<?=$i?> : </label><input type="text" class="form-control"  name="startingtime<?=$i?>"><br>
						<?php } ?>
                    </div>
					...
					<a href="staffaddrouteandschedule.php"><button type="button" class="btn btn-primary">Back</button></a>
					</div>
