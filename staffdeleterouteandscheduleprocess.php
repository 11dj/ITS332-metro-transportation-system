                ...
                <div class="box-header with-border">
				  ...
				  <?php
					...
					<?php
						...
						if($scheduleid=="all"){
							$qdel="delete from route where RouteID='$routeid'";
							$qdelroutename="delete from routenaming where RouteID='$routeid'";
							$resultqdelroutename = $mysqli->query($qdelroutename);
						}
						else{
							$qdel="delete from route where RouteID='$routeid' and ScheduleID='$scheduleid' ";
						}
						$result = $mysqli->query($qdel);
								if (!$result){
									die('Error: '.$insert." ". $mysqli->error);
								}
								else{
									$checkifstillrouteandscheuleremain="select * from route where RouteID='$routeid'";
									$resultchkifremain=$mysqli->query($checkifstillrouteandscheuleremain);
									// print_r($resultchkifremain);
									$rowchkifremain=$resultchkifremain->fetch_array();
									// echo $rowchkifremain['RouteID'];
									if($rowchkifremain['RouteID']==NULL){
										$qdelroutename="delete from routenaming where RouteID='$routeid'";
										$resultqdelroutename = $mysqli->query($qdelroutename);
									}?>
									Delete Success
