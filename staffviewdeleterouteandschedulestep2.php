        ...
        <section class="content">
		  ...
		  <div class="box box-warning">
                <div class="box-header with-border">
                  <h3 class="box-title">View / Edit / Delete Route and Schedule</h3>
                </div><!-- /.box-header -->
                <div class="box-body">
                    ...
                    <div class="form-group">
						...
						if(isset($_GET['routeid'])){
							...
							$rowrouteid=$resultgetrouteid->fetch_array();
							$routename=$rowrouteid['RouteName'];
						}
						else{
							header("Location:staffviewdeleterouteandschedule.php");
						}
					?>
					<label>Select schedule from route <?=$routename?> with RouteID=<?=$routeid?> you want to view</label><br>
					<form action="staffviewdeleterouteandschedulestep3.php">
					<input  type="hidden" name="routeid" value="<?=$routeid?>">
					<select class="form-control" name="scheduleid">
						<option value="all">View All ScheduleID</option>
					<?php
					$getsid="select ScheduleID, DepartTime from route where RouteID='$routeid' and Step='1'";
					$resultgetsid=$mysqli->query($getsid);
					while($rowsid=$resultgetsid->fetch_array()){ ?>
						<option value="<?=$rowsid['ScheduleID']?>">ScheduleID=<?=$rowsid['ScheduleID']?>, DepartTimeForStep1 : <?=$rowsid['DepartTime']?></option>
						<?php } ?>
					</select>
					<br>
                    </div>
